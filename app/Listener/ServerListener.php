...
class ServerListener implements EventListenerInterface
{
    ...
    public function process(object $event): void
    {
        if ($event instanceof OnRequest) {
            ...
            echo sprintf("[MaxPHP] %s |%s| %15s|\033[44m%7s\033[0m| \"%s\"\n", date('Y/m/d H:i:s'), $statusCode, $event->request->getRealIp(), $event->request->getMethod(), $event->request->getUri()->getPath());
        }
    }
}
